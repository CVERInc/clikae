...
//#endregion
//#region node_modules/@junejs/core/src/islands.tsx
var import_client = require_client();
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
const ISLAND_TAG = "june-island";
const ISLAND_NAME_ATTR = "data-june-island";
const ISLAND_PROPS_ATTR = "data-june-props";
const ISLAND_STRATEGY_ATTR = "data-june-strategy";
const ISLAND_SLOT_TAG = "june-slot";
function deserializeIslandProps(raw) {
	if (!raw) return {};
	try {
...
function JuneSlot({ children, html }) {
	if (html != null) return (0, import_react.createElement)(ISLAND_SLOT_TAG, {
		dangerouslySetInnerHTML: { __html: html },
		suppressHydrationWarning: true
	});
	return (0, import_react.createElement)(ISLAND_SLOT_TAG, null, children);
}
//#endregion
//#region node_modules/@junejs/core/src/morph.ts
const isIsland = (n) => n.nodeType === 1 && n.tagName.toLowerCase() === "june-island";
const islandName = (el) => el.getAttribute("data-june-island") ?? "";
const reusableIn = (all) => (n) => isIsland(n) && !!n.__juneHydrated && (all || n.hasAttribute("data-june-persist"));
function placeBefore(parent, node, ref) {
	const mb = parent.moveBefore;
	if (typeof mb === "function" && node.isConnected && node.parentNode === parent) try {
...
function syncAttrs(oldEl, newEl) {
	for (const a of Array.from(oldEl.attributes)) if (!newEl.hasAttribute(a.name)) oldEl.removeAttribute(a.name);
	for (const a of Array.from(newEl.attributes)) if (oldEl.getAttribute(a.name) !== a.value) oldEl.setAttribute(a.name, a.value);
}
const sameType = (a, b) => a.nodeType === b.nodeType && (a.nodeType !== 1 || a.tagName === b.tagName);
const fresh = (parent, n) => parent.ownerDocument.importNode(n, true);
function morph(oldEl, newEl, opts = {}) {
	...
	for (let n = newEl.firstChild; n; n = n.nextSibling) {
		if (isIsland(n)) {
			const name = islandName(n);
			const live = pool.get(name);
			const node = live ?? fresh(oldEl, n);
			if (node !== o) placeBefore(oldEl, node, o);
		...
		if (o && sameType(o, n) && !isIsland(o)) {
			if (n.nodeType === 1) morph(o, n, opts);
			else if (o.nodeValue !== n.nodeValue) o.nodeValue = n.nodeValue;
			o = o.nextSibling;
		} else placeBefore(oldEl, fresh(oldEl, n), o);
	}
	while (o) {
		const next = o.nextSibling;
		if (!reusable(o) || pool.has(islandName(o))) oldEl.removeChild(o);
		o = next;
	}
}
//#endregion
//#region node_modules/@junejs/core/src/nav-protocol.ts
const FRAGMENT_ACCEPT = "text/vnd.june.fragment+html";
const TITLE_HEADER = "x-june-title";
...
const FLIGHT_ACCEPT = "text/vnd.june.flight";
const OUTLET_ATTR = "data-june-outlet";
const SHELL_ATTR = "data-june-shell";
const SEGMENT_HEADER = "x-june-segment";
//#endregion
//#region node_modules/@junejs/core/src/shell.ts
...
const mountedShellKey = () => rootEl()?.getAttribute("data-june-shell") ?? null;
function resolveSwapTarget(fragmentShell) {
	if (fragmentShell === null) return rootEl();
	return fragmentShell === mountedShellKey() ? outletEl() : null;
}
//#endregion
//#region node_modules/@junejs/core/src/client-router.ts
const trimSlash = (p) => p.length > 1 && p.endsWith("/") ? p.slice(0, -1) : p;
let cachedOutlet = null;
let cachedShellLinks = [];
function updateActiveLinks() {
	const outlet = outletEl();
	if (!outlet) return;
	if (cachedOutlet !== outlet) {
		cachedOutlet = outlet;
		cachedShellLinks = Array.from(document.querySelectorAll("a[href]")).filter((a) => !outlet.contains(a) && a.origin === location.origin);
	}
	const here = trimSlash(location.pathname);
	for (const a of cachedShellLinks) {
		const p = trimSlash(a.pathname);
		const exact = p === here;
		if (exact || p !== "/" && here.startsWith(p + "/")) a.setAttribute("aria-current", exact ? "page" : "true");
...
function startClientRouter(rehydrate) {
	const w = window;
...
//#endregion
//#region node_modules/@junejs/core/src/client-live.ts
function applyLiveUpdate(fragmentHtml, title, rehydrate, segmentShell) {
...
//#endregion
//#region node_modules/@junejs/core/src/islands-client.ts
function mount(el, Component) {
	if (el.__juneHydrated) return;
	el.__juneHydrated = true;
	const props = deserializeIslandProps(el.getAttribute(ISLAND_PROPS_ATTR));
	const strategy = el.getAttribute(ISLAND_STRATEGY_ATTR);
	if (el.hasAttribute("data-june-slot")) {
		const slotEl = el.querySelector(ISLAND_SLOT_TAG);
		const html = slotEl ? slotEl.innerHTML : "";
		(0, import_client.hydrateRoot)(el, import_react.createElement(Component, {
			...props,
			children: import_react.createElement(JuneSlot, { html })
		}));
		return;
	}
	if (strategy === "only") (0, import_client.createRoot)(el).render(import_react.createElement(Component, props));
	else (0, import_client.hydrateRoot)(el, import_react.createElement(Component, props));
}
function schedule(el, run) {
	const strategy = el.getAttribute("data-june-strategy") ?? "load";
	if (strategy === "idle" && "requestIdleCallback" in window) window.requestIdleCallback(run);
	else if (strategy === "visible" && "IntersectionObserver" in window) new IntersectionObserver((entries, obs) => {
...
function hydrateIslands(loaders, root = document) {
	const markers = root.querySelectorAll(`${ISLAND_TAG}[${ISLAND_STRATEGY_ATTR}]`);
	let n = 0;
	for (const el of markers) {
		if (el.__juneHydrated) continue;
		...
		schedule(el, () => {
			load().then((Component) => mount(el, Component), (err) => console.error(`[june] failed to load island "${name}":`, err));
		});
		n++;
	}
	return n;
}
function startJuneClient(options) {
	const { loaders } = options;
	const rehydrate = (root) => hydrateIslands(loaders, root);
	rehydrate(document);
	...
	if (routerRoot.getAttribute("data-june-router") === "flight") {
		import("./client-router-flight-ca52e466.js").then(({ startFlightRouter }) => startFlightRouter());
		return;
	}
	startClientRouter(rehydrate);
	window.__juneLiveReload = async () => {
		try {
			const res = await fetch(location.href, { headers: { accept: FRAGMENT_ACCEPT } });
			if (!res.ok) return false;
			return applyLiveUpdate(await res.text(), decodeTitle(res.headers.get(TITLE_HEADER)), rehydrate, res.headers.get(SEGMENT_HEADER));
		} catch {
			return false;
		}
	};
...
//#endregion
//#region node_modules/@kurajs/ctrlk/dist/core.js
...
function defaultFilter(item, query) {
	...
	if (idx >= 0) return 1e3 - idx - hay.length * .01;
	let at = 0;
	for (const ch of q) {
...
function createCtrlk(options = {}) {
	...
	let ac = null;
	let runSeq = 0;
	let pendingValue = options.value ?? null;
	const emit = () => {
		const snap = state;
	...
	const runStatic = (q) => {
		...
		applyItems(staticItems.map((it) => ({
			it,
			s: filter(it, q)
		})).filter((x) => x.s > 0).sort((a, b) => b.s - a.s).map((x) => x.it), true);
	};
	const runAsync = async (q) => {
		...
		try {
			...
			if (seq === runSeq) applyItems(items, true);
		} catch (err) {
			if (seq === runSeq && !controller.signal.aborted) set({
				loading: false,
				error: err
			});
		}
...
//#endregion
//#region node_modules/@kurajs/ctrlk/dist/highlight.js
/**
...
//#endregion
//#region node_modules/@kurajs/ctrlk/dist/styles.js
const STYLE_ID = "ctrlk-styles";
const CSS = `
.ctrlk-overlay {
  --ctrlk-bg: #fff;
  --ctrlk-fg: #1a1a1a;
...
function injectStyles(doc = document) {
	...
	if (doc.getElementById("ctrlk-styles")) return;
...
function platformHotkeyLabel(platform) {
...
function mountCtrlk(ctrl, opts = {}) {
	...
	function fillHighlighted(el, text, tokens) {
		el.textContent = "";
		for (const seg of highlight(text, tokens)) if (seg.match) {
	...
	function rebuildList(state) {
		...
		for (const group of state.groups) {
			...
			for (const item of group.items) {
				const index = flat++;
				const base = opts.renderItem ? opts.renderItem(item, {
					active: false,
					tokens
				}) : defaultRow(item, tokens);
				...
				base.addEventListener("mousemove", () => {
					if (state.activeIndex !== index) ctrl.setActive(index);
				});
				list.append(base);
				optionEls.push(base);
			}
	...
	const onOverlayMousedown = (e) => {
		if (e.target === overlay) ctrl.close();
	};
	...
	const triggers = [];
	if (opts.trigger) {
		const els = typeof opts.trigger === "string" ? Array.from(doc.querySelectorAll(opts.trigger)) : opts.trigger instanceof Element ? [opts.trigger] : Array.from(opts.trigger);
		for (const el of els) {
			triggers.push(el);
...
//#endregion
//#region node_modules/@kurajs/docs/dist/client.js
...
function setup(opts) {
	const trigger = document.querySelector(opts.trigger ?? ".search-box");
...
function addHint(trigger) {
	...
	trigger.style.paddingRight = "3.4rem";
	host.appendChild(kbd);
}
//#endregion
//#region .june/routes/_client.ts
startJuneClient({ loaders: {} });
initSearch();
//#endregion
export { require_react as a, require_client as i, TITLE_HEADER as n, __toESM as o, decodeTitle as r, FLIGHT_ACCEPT as t };
