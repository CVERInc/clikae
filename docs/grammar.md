...
| `clikae usage [engine] [tank] [--json] [--fresh]` | Each tank's vendor usage window(s): used% and reset time, cached 120s (`CLIKAE_USAGE_TTL`; `--fresh` bypasses it). `source` is `vendor` (a live call answered), `transcript` (read from evidence the engine already wrote locally — no live process invoked; currently Codex, from the same rollout evidence `limit_codex_status` uses), or `unknown`. The board reads this cache to show cached percentages and never calls the vendor itself; `clikae burn` never does either — its reroute ranking reads whatever is already cached (stale allowed) and leaves fetching to this command. |
