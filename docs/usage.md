...
| `resume cleanup [--dry-run] [--older-than <days>] [--min-size <MB>]` | Delete session transcripts/databases to free disk space. Interactive — a checkbox preview, biggest first (space toggles a row, `a` all, Enter proceeds), then a final confirm; never touches tank configs, memory, or settings. Two filters pick the regular candidates: `--older-than` (default 30) by age, `--min-size` by size — given alone, `--min-size` drops the age cutoff (space usually lives in big *recent* sessions). Every run **also** offers two kinds of pure waste, regardless of the filters: **stale copies** (`to`/relay and a cross-tank resume *copy* the session and never clean the source — copies are grouped per session across all tanks, the largest is kept, and a copy is auto-selected only when it's provably contained in the kept one; a copy with unique content shows as "diverged" and starts unchecked) and **orphaned subagent data** (claude's leftover `<sid>/` sibling dirs; deleting a transcript now removes its sibling dir too). `--dry-run` prints the same list without deleting. |
