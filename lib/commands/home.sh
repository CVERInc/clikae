...
_home_fuel_dotv_compute() {
  ...
  # ⏳ and the remedy. Not ⏳ as the dot: every dot is one column wide and the
  ...
  if [ -f "$_ucache" ]; then
    ...
    case "$_uline" in
      *'"source":"expired"'*)
        ...
        if [ -n "$_uca" ] && [ $(( now - _uca )) -lt 86400 ] && declare -F usage_expired_hintv >/dev/null 2>&1; then
          usage_expired_hintv "$profile"
