...
T_SOUL_NOBRAIN="在船團裡卻沒有共享腦：%s （clikae memory share 加入，或 clikae solo 讓它正式獨立）"
...
T_CLEAN_TRASH_UNAVAILABLE="無法移到垃圾桶 — 原地保留：%s"
T_CLEAN_SKIPPED_N="有 %d 個項目原地保留，沒有刪除任何東西。"
T_CLEAN_TRASH_UNUSABLE="垃圾桶無法使用，因此什麼都沒有更動。"
