...
T_CONTINUE_IN="接續 (Resume) — 在 %s"
T_LIVE="執行中"
T_LIVE_RESUMING="還有"
T_LIVE_ENTER="按 Enter 回到裡面"
T_RESUME_FOOTER="共 %d 個 session · 按 [R] 搜尋或查看全部"
T_RESUME_TRUNCATED="%d 個 session 被當成 burn 隱藏 · 清單已截斷 — 請用：clikae resume --all"
T_RESUME_ELSEWHERE="這個 store 還有 %d 個 — 請用：clikae resume（不限目錄）"
T_BURN_RESUMES_AT="%s 的 burn 將於 %s 繼續"
T_TANKS="油箱"
T_SOLO_SECTION="單飛 (Solo)"
T_LANG_PICK="選擇介面語言"
T_AUTONOMY_PICK="油盡時的自主層級"
T_RESUME="接回"
T_ENTER_RESUME="Enter 接回"
T_ALSO_AVAILABLE="也可開啟"
T_SOUL_NOBRAIN="在艦隊裡卻沒有共享腦：%s（clikae memory share 加入，或 clikae solo 讓它正式獨立）"
...
T_K_OPEN="開啟"
T_K_RELAY="接力"
T_K_RESUME_ALL="所有 session — 搜尋全部油箱並接續"
T_K_NEW="新增"
T_K_RENAME="改名"
T_K_DELETE="刪除"
T_K_SOLO="單飛／歸隊（移出艦隊 — 不接力／burn／共享）"
...
T_K_TOGGLE_ALL="切換 --all"
T_K_TOGGLE_BURN="切換顯示 burn session"
...
T_CLEAN_TRASH_UNAVAILABLE="無法移至垃圾桶 — 保留於原位：%s"
T_CLEAN_SKIPPED_N="已保留 %d 個項目，未刪除任何內容。"
T_CLEAN_TRASH_UNUSABLE="垃圾桶無法使用，因此未進行任何變更。"
T_CLEAN_TRASH_UNUSABLE_HINT="clikae clean 只會把 session「移到」~/.Trash，永遠不會直接刪除。請修復或清空 ~/.Trash 後再執行一次。"
...
T_CLEAN_ARCHIVE_WITHHELD="%s 個工作階段未列出：在最後一次歸檔後才寫入（%s）。"
T_CLEAN_ARCHIVE_ABSENT="找不到歸檔標記（%s）— 不會列出任何工作階段資料。可用 --no-archive-check 覆寫。"
T_CLEAN_ARCHIVE_REASON_MISSING="尚無標記檔案"
T_CLEAN_ARCHIVE_REASON_UNREADABLE="標記檔案無法讀取"
T_CLEAN_ARCHIVE_REASON_NONNUMERIC="標記不是有效的時間戳記"
T_CLEAN_ARCHIVE_REASON_FUTURE="標記的時間戳記在未來"
T_K_HELP="說明"
T_K_LANG="語言"
T_K_TOPBOTTOM="頂/底"
T_K_JUMP="跳到第 N 個"
...
T_K_CLOSE="關閉"
T_CLOSE_ASK="關閉這個執行中的 session？對話仍然 resume 得回來。[y/N] "
