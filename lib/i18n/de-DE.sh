# shellcheck shell=bash
# shellcheck disable=SC2034  # T_* are a string table consumed by the renderers.
# lib/i18n/de-DE.sh — Deutsch (German). Loaded OVER the en-US base by i18n_load,
# and must define every key lib/i18n/en-US.sh defines (tests/bats/i18n.bats
# enforces it, including printf placeholder parity). Same file contract as
# en-US.sh: one `T_KEY="value"` per line at column 0, %s/%d kept in en-US's
# order, %% for a literal percent.
#
# Voice: Apple's German macOS strings — du, never Sie; verb-first button labels;
# accepted loanwords (Update, Login, Engine) over compound-noun monsters, to
# keep the column-aligned board tight.

T_LANG_NAME="Deutsch"

# board
T_WORDMARK="clikae"                          # deliberately the plain wordmark (the katakana bonus is ja-JP-only)
T_TAGLINE1="jedes CLI zwischen Accounts wechseln"
T_TAGLINE2="— Tank tauschen, weiterbrennen"
T_CONTINUE="Fortsetzen"
T_CONTINUE_IN="Fortsetzen — in %s"
T_LIVE="Aktiv"
T_LIVE_RESUMING="fortgesetzt in"
T_LIVE_ENTER="Enter zum Zurückkehren"
T_RESUME_FOOTER="%d Sitzungen insgesamt · [R] für alle / Suche"
T_RESUME_TRUNCATED="%d Sitzungen als Burn-Läufe ausgeblendet · Liste gekürzt — siehe: clikae resume --all"
T_RESUME_ELSEWHERE="%d weitere in diesem Speicher — siehe: clikae resume (beliebiges Verzeichnis)"
T_BURN_RESUMES_AT="%s-Burn wird um %s fortgesetzt"
T_TANKS="Tanks"
T_SOLO_SECTION="Solo"
T_LANG_PICK="Anzeigesprache"
T_AUTONOMY_PICK="Autonomie bei leerem Tank"
T_RESUME="weiter"
T_ENTER_RESUME="Enter: fortsetzen"
T_ALSO_AVAILABLE="Auch verfügbar"
T_SOUL_NOBRAIN="in der Flotte, aber ohne gemeinsames Hirn: %s  (clikae memory share, oder clikae solo, wenn es so bleiben soll)"
T_NO_TANK_DEFAULT="noch kein Tank — öffnet Standard"
T_AGY_NOTE="Einzel-Account · globales Login (ein Account, alle Shells)"
T_AGY_BURN_NOTE="Das agy-Login ist global (immer nur ein Account, nie zwei parallel) — 'clikae burn agy <tank>' kann es bei leerem Tank der Reihe nach auf den nächsten umleiten (Keychain-Übernahme, kein OAuth nötig), oder du führst es per 'agy -p' headless auf dem aktiven Account aus."
T_LAUNCH="starten"
T_MORE="mehr"
T_OVER_QUOTA="Limit erreicht"
T_OVER_QUOTA_HINT="nimm deine Sitzung zum nächsten Tank mit:  clikae to"
# footer key hints
T_K_MOVE="bewegen"
T_K_OPEN="öffnen"
T_K_RELAY="Übergabe"
T_K_RESUME_ALL="alle Sitzungen — über alle Tanks suchen und fortsetzen"
T_K_NEW="neu"
T_K_RENAME="umbenennen"
T_K_DELETE="löschen"
T_K_SOLO="Solo an/aus (raus aus der Flotte — keine Übergabe/Burn/Teilen)"
T_K_MEMORY="Gedächtnis (Soul) — das Hirn dieses Tanks teilen / isolieren"
T_MEM_TITLE="Gedächtnis (Soul)"
T_MEM_OPT_SHARE="in eine Gruppe teilen…"
T_MEM_OPT_ISOLATE="isolieren (eigenes Gedächtnis)"
T_MEM_OPT_STATUS="Status (Teilen anzeigen)"
T_MEM_SHARE_FOR="Gedächtnis teilen für"
T_MEM_GROUP_PROMPT="Gruppenname: "
T_MEM_NOGROUP="Keine Gruppe angegeben — abgebrochen."
T_K_QUIT="beenden"
T_K_FILTER="Filter"
T_K_CLEANUP="aufräumen"
T_K_CLEAN="Sitzungsdaten aufräumen — Speicherplatz freigeben"
T_K_TOGGLE_ALL="--all umschalten"
T_K_TOGGLE_BURN="Burn-Sitzungen ein-/ausblenden"
# `clikae clean` section headings (T_CLEAN_SECT_OLD/_MIN are printf formats)
T_CLEAN_SECT_REDUNDANT="Redundant (sicher)"
T_CLEAN_SECT_OLD="Seit %s+ Tagen unberührt"
T_CLEAN_SECT_MIN="%s MB oder größer"
T_CLEAN_SECT_BIG="Groß, aber frisch — du entscheidest"
T_CLEAN_LBL_STALE="alte Kopie (behalten: %s)"
T_CLEAN_LBL_ORPHAN="verwaiste Subagent-Daten"
T_CLEAN_LBL_DIVERGED="abweichend — eigener Inhalt"
T_CLEAN_NO_TRANSCRIPT="(kein Protokoll)"
T_CLEAN_NO_PREVIEW="(keine Vorschau)"
T_CLEAN_AGE_AGO="vor %sT"
T_CLEAN_LIST_HEADING="Sitzungsdaten, die aufgeräumt werden können (größte zuerst je Abschnitt):"
T_CLEAN_TOTAL="Sitzungen zum Aufräumen: %s"
T_CLEAN_SELECTED="Ausgewählte Sitzungen: %s"
T_CLEAN_SPACE_TO_FREE="Geschätzter Speicherplatz, der in den Papierkorb wandert: %s"
T_CLEAN_UNCHECKED_HINT="%s Zeile(n) starten unmarkiert — groß, aber frisch, und abweichende Kopien: du entscheidest."
T_CLEAN_DRYRUN_NOTE="Dry-Run-Modus: keine Dateien verschoben."
T_CLEAN_REFUSE_NONINTERACTIVE="Verschieben in den Papierkorb ohne interaktive Bestätigung verweigert — führe den Befehl in einem Terminal erneut aus (oder nutze --dry-run zur Vorschau)."
T_CLEAN_CANCELLED="Abgebrochen — nichts in den Papierkorb verschoben."
T_CLEAN_NOTHING_SELECTED="Nichts ausgewählt — nichts in den Papierkorb verschoben."
T_CLEAN_CONFIRM_Q="Diese Sitzungen wirklich in den Papierkorb verschieben?"
T_CLEAN_CONFIRM_PROMPT="Enter zum Fortfahren, Ctrl-C zum Abbrechen: "
T_CLEAN_NO_CONFIRM="Keine Bestätigung erhalten — nichts in den Papierkorb verschoben."
T_CLEAN_DELETING="Sitzungsdateien werden in den Papierkorb verschoben..."
T_CLEAN_DONE="Aufräumen abgeschlossen. Ca. %s in den Papierkorb verschoben — erst das Leeren des Papierkorbs gibt den Speicherplatz wirklich frei."
T_CLEAN_TRASH_UNAVAILABLE="Konnte nicht in den Papierkorb verschoben werden — verbleibt am ursprünglichen Ort: %s"
T_CLEAN_SKIPPED_N="%d Objekt(e) verblieben an ihrem Ort. Es wurde nichts gelöscht."
T_CLEAN_TRASH_UNUSABLE="Der Papierkorb ist nicht nutzbar, daher wurde nichts verändert."
T_CLEAN_TRASH_UNUSABLE_HINT="clikae clean VERSCHIEBT Sitzungen nur nach ~/.Trash — es löscht sie niemals. Repariere oder leere ~/.Trash und führe den Befehl erneut aus."
T_CLEAN_NONE_MINSIZE="Keine Sitzungen mit mindestens %s MB gefunden (0 Dateien zum Aufräumen)."
T_CLEAN_NONE_AGE_MINSIZE="Keine Sitzungen älter als %s Tage und mindestens %s MB gefunden (0 Dateien zum Aufräumen)."
T_CLEAN_NONE_ALL="Nichts aufzuräumen — keine redundanten Kopien, keine Sitzungen älter als %s Tage, keine über %s MB."
T_CLEAN_PICKER_HINT="· ↑↓ bewegen · space markieren · a alle · ⏎ Auswahl in den Papierkorb · q abbrechen"
T_CLEAN_TALLY="ausgewählt: %s von %s · %s in den Papierkorb"
T_CLEAN_MORE_ABOVE="▲ ... %s mehr oben ..."
T_CLEAN_MORE_BELOW="▼ ... %s mehr unten ..."
T_CLEAN_ERR_OLDER_THAN="Fehler: --older-than braucht eine Zahl (Tage)."
T_CLEAN_ERR_MIN_SIZE="Fehler: --min-size braucht eine Zahl (Megabyte)."
T_CLEAN_ERR_UNKNOWN_ARG="Unbekanntes Argument: %s  (siehe: clikae clean --help)"
T_CLEAN_ARCHIVE_WITHHELD="%s Sitzung(en) nicht angeboten: nach dem letzten Archivierungslauf geschrieben (%s)."
T_CLEAN_ARCHIVE_ABSENT="Kein Archiv-Marker gefunden (%s) — es werden keine Sitzungsdaten angeboten. Mit --no-archive-check überschreiben."
T_CLEAN_ARCHIVE_REASON_MISSING="noch keine Markerdatei"
T_CLEAN_ARCHIVE_REASON_UNREADABLE="die Markerdatei kann nicht gelesen werden"
T_CLEAN_ARCHIVE_REASON_NONNUMERIC="der Marker ist kein gültiger Zeitstempel"
T_CLEAN_ARCHIVE_REASON_FUTURE="der Zeitstempel des Markers liegt in der Zukunft"
T_K_HELP="Hilfe"
T_K_LANG="Sprache"
T_K_TOPBOTTOM="Anfang/Ende"
T_K_JUMP="zu N springen"
T_K_REORDER="ordnen"                         # terser than "sortieren" — the compact footer needed the columns
T_K_AUTO="Autonomie"
T_K_INCOGNITO="Inkognito"
# welcome
T_NO_TANKS_YET="Noch keine Tanks"
T_ENGINES_HERE="%s Engines, hier:"
T_ENGINES_SUPPORTED="%s Engines unterstützt"
T_NONE_DETECTED="(keine im PATH gefunden)"
T_FILL_FIRST="Fülle deinen ersten Tank:"
T_CURIOUS_DEMO="Neugierig?  clikae demo"
# resume submenu
T_RESUME_TITLE="Diese Sitzung — wie weiter?"
T_RESUME_OPT_RESUME="Dort fortfahren, wo du aufgehört hast"
T_RESUME_OPT_SWITCH="Diesen Tank frisch öffnen (nicht fortsetzen)"
T_RESUME_DRY_TITLE="%s ist leergebrannt — wie weiter?"
T_RESUME_OPT_RELAY="Diese Sitzung zu %s mitnehmen"
T_RESUME_OPT_FORCE="%s trotzdem fortsetzen (läuft ins Limit)"
T_RESUME_OPT_CARRY="Diese Sitzung in einen anderen Tank mitnehmen"
T_RESUME_CARRY_PICK="%s mitnehmen — Tank zum Weitermachen wählen"
T_RESUME_WHICH_TANK="Auf welchem Tank fortsetzen?"
T_UPDATE_AVAIL="Update verfügbar!"
T_UPDATE_NOTES="Release Notes:"
T_UPDATE_NOW="Jetzt aktualisieren (führt \`%s\` aus)"
T_UPDATE_SHOW="Update-Befehl anzeigen"
T_UPDATE_SKIP="Überspringen"
T_UPDATE_SKIP_VER="Bis zur nächsten Version überspringen"
T_UPDATE_DONE="Aktualisiert — starte clikae neu für die neue Version."
T_UPDATE_FAILED="Update-Befehl fehlgeschlagen — führe ihn selbst aus oder sieh auf der Release-Seite nach."
T_UPDATE_MANUAL="Aktualisiere clikae mit deinem Installer, oder hol es hier:"
T_DRY_SEEN="Stand %s"
# new-tank / rename prompts
T_NEWTANK_TITLE="Neuer Tank — CLI wählen"
T_NEWTANK_PROFILE="Tankname für %s (z. B. work, personal): "
T_NEWTANK_CANCEL="Abgebrochen — kein Tank erstellt."
T_NEWTANK_NONAME="Abgebrochen — kein Name angegeben."
T_RENAME_FOR="Umbenennen"
T_RENAME_NEW="Neuer Name: "
T_RENAME_CANCEL="Abgebrochen — Name unverändert."
# filter / help / misc
T_FILTER_PROMPT="Filter: "
T_FILTER_NONE="keine Treffer"
T_HELP_TITLE="clikae — Tasten"
T_HELP_AGY="agy (Antigravity) ist der Power-Modus: 'n' → agy oder 'clikae init agy <name>', übernimmt ~/.gemini (fragt vorher)."
T_DOTS_TITLE="Punkte = Sprit"
T_DOT_READY="bereit"
T_DOT_DRY="leer (über Limit)"
T_DOT_WEEK="Wochen-% (BETA)"
T_DOT_NONE="keine Anzeige"
T_HELP_DISMISS="beliebige Taste schließt"
T_PICKER_HINT="auf/ab bewegen · Enter wählen · q abbrechen"
T_LANG_SET="Anzeigesprache: %s"
T_LANG_UNKNOWN="Unbekannte Sprache: %s  (verfügbar: %s)"

# German pluralises Tank/Engine with a plain -s, but "across" needs rephrasing —
# override the English summary ("3 Tanks für 2 Engines").
i18n_summary() {
  local n="$1" m="$2"
  printf '%s Tank%s für %s Engine%s' \
    "$n" "$([ "$n" = 1 ] || echo s)" "$m" "$([ "$m" = 1 ] || echo s)"
}
T_K_CLOSE="schließen"
T_CLOSE_ASK="Diese laufende Sitzung schließen? Das Gespräch bleibt fortsetzbar. [y/N] "
