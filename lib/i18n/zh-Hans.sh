# shellcheck shell=bash
# shellcheck disable=SC2034  # T_* are a string table consumed by the renderers.
# lib/i18n/zh-Hans.sh — 简体中文（Simplified Chinese）. Chinese is keyed by WRITING
# SYSTEM, not region: this file serves zh_CN / zh_SG / anything *Hans*; Traditional
# (zh-TW — zh_TW / zh_HK / *Hant*) is a SEPARATE locale file. Loaded OVER the
# en-US base by i18n_load and must define every key lib/i18n/en-US.sh defines
# (tests/bats/i18n.bats enforces it, including printf placeholder parity).
# Same file contract as en-US.sh: one `T_KEY="value"` per line at column 0,
# %s/%d kept in en-US's order, %% for a literal percent.

T_LANG_NAME="简体中文"

# board
T_WORDMARK="clikae"                          # deliberately the plain wordmark (the katakana bonus is ja-JP-only)
T_TAGLINE1="在帐户之间切换任何 CLI"
T_TAGLINE2="— 换个油箱，继续燃烧"
T_CONTINUE="继续 (Resume)"
T_CONTINUE_IN="继续 (Resume) — 在 %s"
T_LIVE="运行中"
T_LIVE_RESUMING="还有"
T_LIVE_ENTER="按 Enter 回到里面"
T_RESUME_FOOTER="共 %d 个会话 · 按 [R] 搜索或查看全部"
T_RESUME_TRUNCATED="%d 个会话被当作 burn 隐藏 · 列表已截断 — 请用：clikae resume --all"
T_RESUME_ELSEWHERE="这个 store 还有 %d 个 — 请用：clikae resume（不限目录）"
T_BURN_RESUMES_AT="%s 的 burn 将于 %s 继续"
T_TANKS="油箱"
T_SOLO_SECTION="单飞 (Solo)"
T_LANG_PICK="选择界面语言"
T_AUTONOMY_PICK="油尽时的自主级别"
T_RESUME="恢复"
T_ENTER_RESUME="Enter 恢复"
T_ALSO_AVAILABLE="也可打开"
T_SOUL_NOBRAIN="在舰队里却没有共享大脑：%s（clikae memory share 加入，或 clikae solo 让它正式独立）"
T_NO_TANK_DEFAULT="还没有油箱 — 按默认打开"
T_AGY_NOTE="单帐户 · 全局登录（所有 shell 共用一个帐户）"
T_AGY_BURN_NOTE="agy 的登录是全局的（同一时间只有一个帐户，无法并行）— 'clikae burn agy <tank>' 能在烧干时按顺序接力到下一个油箱（Keychain 携带登录，不必 OAuth）；也能用 'agy -p' 在当前帐户跑 headless。"
T_LAUNCH="启动"
T_MORE="更多"
T_OVER_QUOTA="已达上限"
T_OVER_QUOTA_HINT="把会话接到下一个油箱：clikae to"
# footer key hints
T_K_MOVE="移动"
T_K_OPEN="打开"
T_K_RELAY="接力"
T_K_RESUME_ALL="所有会话 — 搜索全部油箱并继续"
T_K_NEW="新建"
T_K_RENAME="重命名"
T_K_DELETE="删除"
T_K_SOLO="单飞／归队（移出舰队 — 不参与接力／burn／共享）"
T_K_MEMORY="记忆（Soul）— 共享／隔离这个油箱的大脑"
T_MEM_TITLE="记忆（Soul）"
T_MEM_OPT_SHARE="共享进一个分组…"
T_MEM_OPT_ISOLATE="隔离（用自己的记忆）"
T_MEM_OPT_STATUS="状态（显示共享情况）"
T_MEM_SHARE_FOR="共享记忆"
T_MEM_GROUP_PROMPT="分组名称："
T_MEM_NOGROUP="没有输入分组名 — 已取消。"
T_K_QUIT="退出"
T_K_FILTER="筛选"
T_K_CLEANUP="清理"
T_K_CLEAN="清理会话数据 — 释放磁盘空间"
T_K_TOGGLE_ALL="切换 --all"
T_K_TOGGLE_BURN="切换显示 burn 会话"
T_CLEAN_SECT_REDUNDANT="冗余数据（可安全删除）"
T_CLEAN_SECT_OLD="超过 %s 天未使用"
T_CLEAN_SECT_MIN="%s MB 及以上"
T_CLEAN_SECT_BIG="很大但最近用过 — 由你决定"
T_CLEAN_LBL_STALE="冗余副本（保留 %s 的那份）"
T_CLEAN_LBL_ORPHAN="孤立子代理数据"
T_CLEAN_LBL_DIVERGED="分歧 — 含独有内容"
T_CLEAN_NO_TRANSCRIPT="（无记录）"
T_CLEAN_NO_PREVIEW="（无预览）"
T_CLEAN_AGE_AGO="%s天前"
T_CLEAN_LIST_HEADING="可清理的会话数据（各区块内由大到小）："
T_CLEAN_TOTAL="可清理会话数：%s"
T_CLEAN_SELECTED="已选中会话数：%s"
T_CLEAN_SPACE_TO_FREE="预计移入废纸篓的空间：%s"
T_CLEAN_UNCHECKED_HINT="%s 行默认未勾选 — 大但较新的会话与分歧副本，由你决定。"
T_CLEAN_DRYRUN_NOTE="Dry-run 模式：未移动任何文件。"
T_CLEAN_REFUSE_NONINTERACTIVE="没有交互式确认就不会移入废纸篓 — 请在终端重新运行，或用 --dry-run 预览。"
T_CLEAN_CANCELLED="已取消 — 未移入废纸篓任何内容。"
T_CLEAN_NOTHING_SELECTED="未选中任何项 — 未移入废纸篓任何内容。"
T_CLEAN_CONFIRM_Q="确定要将这些会话移入废纸篓吗？"
T_CLEAN_CONFIRM_PROMPT="按 Enter 继续，或 Ctrl-C 取消："
T_CLEAN_NO_CONFIRM="未收到确认 — 未移入废纸篓任何内容。"
T_CLEAN_DELETING="正在将会话文件移入废纸篓…"
T_CLEAN_DONE="清理完成，已将约 %s 移入废纸篓 — 清空废纸篓才会真正释放空间。"
T_CLEAN_TRASH_UNAVAILABLE="无法移至废纸篓 — 保留在原处：%s"
T_CLEAN_SKIPPED_N="已保留 %d 个项目，未删除任何内容。"
T_CLEAN_TRASH_UNUSABLE="废纸篓无法使用，因此未作任何更改。"
T_CLEAN_TRASH_UNUSABLE_HINT="clikae clean 只会把会话「移到」~/.Trash，永远不会直接删除。请修复或清空 ~/.Trash 后再执行一次。"
T_CLEAN_NONE_MINSIZE="没有找到 %s MB 及以上的会话（0 个可清理）。"
T_CLEAN_NONE_AGE_MINSIZE="没有找到超过 %s 天且达 %s MB 的会话（0 个可清理）。"
T_CLEAN_NONE_ALL="没有可清理的 — 无冗余副本、无超过 %s 天的会话、也没有超过 %s MB 的。"
T_CLEAN_PICKER_HINT="· ↑↓ 移动 · space 勾选 · a 全选 · ⏎ 移入废纸篓 · q 取消"
T_CLEAN_TALLY="已选 %s／%s · 废纸篓 %s"
T_CLEAN_MORE_ABOVE="▲ … 上方还有 %s 项 …"
T_CLEAN_MORE_BELOW="▼ … 下方还有 %s 项 …"
T_CLEAN_ERR_OLDER_THAN="错误：--older-than 需要一个数字天数。"
T_CLEAN_ERR_MIN_SIZE="错误：--min-size 需要一个数字 MB 数。"
T_CLEAN_ERR_UNKNOWN_ARG="未知参数：%s（见：clikae clean --help）"
T_CLEAN_ARCHIVE_WITHHELD="%s 个会话未列出：在最后一次归档后才写入（%s）。"
T_CLEAN_ARCHIVE_ABSENT="未找到归档标记（%s）— 不会列出任何会话数据。可用 --no-archive-check 覆盖。"
T_CLEAN_ARCHIVE_REASON_MISSING="尚无标记文件"
T_CLEAN_ARCHIVE_REASON_UNREADABLE="标记文件无法读取"
T_CLEAN_ARCHIVE_REASON_NONNUMERIC="标记不是有效的时间戳"
T_CLEAN_ARCHIVE_REASON_FUTURE="标记的时间戳在未来"
T_K_HELP="帮助"
T_K_LANG="语言"
T_K_TOPBOTTOM="顶部/底部"
T_K_JUMP="跳到第 N 个"
T_K_REORDER="排序"
T_K_AUTO="自主度"
T_K_INCOGNITO="无痕"
# welcome
T_NO_TANKS_YET="还没有油箱"
T_ENGINES_HERE="%s个引擎，就在这里："
T_ENGINES_SUPPORTED="%s个引擎受支持"
T_NONE_DETECTED="（此处 PATH 上未检测到）"
T_FILL_FIRST="装满你的第一个油箱："
T_CURIOUS_DEMO="好奇？  clikae demo"
# resume submenu
T_RESUME_TITLE="这个会话 — 接下来？"
T_RESUME_OPT_RESUME="从上次离开的地方继续"
T_RESUME_OPT_SWITCH="这个油箱、开新局（不接旧进度）"
T_RESUME_DRY_TITLE="%s 已烧干 — 接下来？"
T_RESUME_OPT_RELAY="接到 %s 继续"
T_RESUME_OPT_FORCE="硬要恢复 %s（会立刻撞上限额）"
T_RESUME_OPT_CARRY="带上这段会话，换到另一个油箱"
T_RESUME_CARRY_PICK="带上 %s 换油箱 — 选一个继续"
T_RESUME_WHICH_TANK="要在哪个油箱上恢复？"
T_UPDATE_AVAIL="有新版本！"
T_UPDATE_NOTES="更新说明："
T_UPDATE_NOW="立即更新（执行 \`%s\`）"
T_UPDATE_SHOW="显示更新命令"
T_UPDATE_SKIP="跳过"
T_UPDATE_SKIP_VER="跳过这一版，出更新版本再提醒"
T_UPDATE_DONE="已更新 — 重新运行 clikae 即可生效。"
T_UPDATE_FAILED="更新命令失败 — 请手动执行，或查看 release 页面。"
T_UPDATE_MANUAL="用你的安装方式更新 clikae，或从这里获取："
T_DRY_SEEN="获取于 %s"
# new-tank / rename prompts
T_NEWTANK_TITLE="新建油箱 — 选一个 CLI"
T_NEWTANK_PROFILE="%s 的油箱名称（如：work、personal）："
T_NEWTANK_CANCEL="已取消 — 未创建油箱。"
T_NEWTANK_NONAME="已取消 — 未输入名称。"
T_RENAME_FOR="重命名"
T_RENAME_NEW="新名称："
T_RENAME_CANCEL="已取消 — 名称保持不变。"
# filter / help / misc
T_FILTER_PROMPT="筛选："
T_FILTER_NONE="无匹配项"
T_HELP_TITLE="clikae — 按键"
T_HELP_AGY="agy (Antigravity) 是 power 模式：'n' → agy 或 'clikae init agy <名称>'，会接管 ~/.gemini（会先问你）。"
T_DOTS_TITLE="圆点 = 油量"
T_DOT_READY="有油"
T_DOT_DRY="烧干（超限）"
T_DOT_WEEK="本周用量（BETA）"
T_DOT_NONE="无读数"
T_HELP_DISMISS="按任意键关闭"
T_PICKER_HINT="上下移动 · Enter 选择 · q 取消"
T_LANG_SET="界面语言：%s"
T_LANG_UNKNOWN="未知语言：%s （可用：%s）"

# Chinese measures with 个, no plural forms — override the English summary.
i18n_summary() {
  printf '%s 个油箱、%s 个引擎' "$1" "$2"
}
T_K_CLOSE="关闭"
T_CLOSE_ASK="关闭这个运行中的 session？对话仍然 resume 得回来。[y/N] "
